...
CAMERA_PORT = 80
CAMERA_USER = 'admin'
CAMERA_PASS = 'ErenEnerji' # Güvenlik için gerçek şifrenizi kodda tutmayın
        ...
        try:
            ...
            while self._run_flag:
                ...
                if ret:
                    ...
                    self.connection_status_signal.emit(f"{self.stream_name}: Yeniden Bağlanılıyor...")
                    time.sleep(2)
                    cap.release()
                    cap = cv2.VideoCapture(self.rtsp_url, cv2.CAP_FFMPEG)
            cap.release()
...
class ThermalDataThread(QThread):
    ...
    def run(self):
        try:
            ...
            print(f"Termal Veri Thread Hatası: {e}")
...
class PTZControlApp(QWidget):
    ...
    def init_onvif(self):
        try:
            ...
            ptz_config = self.ptz.GetConfigurationOptions({'ConfigurationToken': self.profile.PTZConfiguration.token})
            pan_limits = ptz_config.Spaces.AbsolutePanTiltPositionSpace[0].XRange
            tilt_limits = ptz_config.Spaces.AbsolutePanTiltPositionSpace[0].YRange
            self.ptz_limits.update({'pan_min': pan_limits.Min, 'pan_max': pan_limits.Max, 'tilt_min': tilt_limits.Min, 'tilt_max': tilt_limits.Max})
            print(f"ONVIF bağlantısı başarılı. PTZ Limitleri: Pan {self.ptz_limits['pan_min']}-{self.ptz_limits['pan_max']}, Tilt {self.ptz_limits['tilt_min']}-{self.ptz_limits['tilt_max']}")
        except Exception as e:
    ...
    def init_ui(self):
        ...
        ptz_directional_layout.addWidget(self.create_ptz_button("←", -0.05, 0), 1, 0)
        self.rotate_button = self.create_rotate_button("⟳ 360")
        ptz_directional_layout.addWidget(self.rotate_button, 1, 1)
        ptz_directional_layout.addWidget(self.create_ptz_button("→", 0.05, 0), 1, 2)
        ...
        ptz_absolute_layout.addRow(f"Pan [{self.ptz_limits['pan_min']:.1f}°, {self.ptz_limits['pan_max']:.1f}°]:", self.pan_input)
        ptz_absolute_layout.addRow(f"Tilt [{self.ptz_limits['tilt_min']:.1f}°, {self.ptz_limits['tilt_max']:.1f}°]:", self.tilt_input)
    ...
    def update_thermal_data(self, data):
        try:
            ...
            self.last_max_temp = max_temp if max_temp is not None else 0.0
            ...
            if hotspot_node:
                self.hotspot_coords = (hotspot_node.get('positionX', 0), hotspot_node.get('positionY', 0))
        except Exception as e:
    ...
    def closeEvent(self, event):
        ...
        self.thread_thermal_data.stop() # Bu thread'i de durdur
