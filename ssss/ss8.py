...
# HATA DÜZELTME: OpenCV'nin RTSP için TCP kullanmasını sağla
            ...
            try:
                ...
                while self._run_flag:
                    ...
                    if not self.is_thermal:
                        ...
                        if self.parent_ui.calibrated_hotspot_coords:
                            x, y = self.parent_ui.calibrated_hotspot_coords
                        ...
                        if self.parent_ui.calibrated_coldspot_coords:
                            x, y = self.parent_ui.calibrated_coldspot_coords
                            px, py = int(x * w_frame), int(y * h_frame)
...
class PTZControlApp(QWidget):
    ...
    def load_initial_thermal_rules(self):
        ...
        try:
            ...
            else: print(f"Kural yükleme hatası: {response.status_code}")
        except Exception as e: print(f"Hedef renklendirme kuralları yüklenemedi: {e}")
    ...
    def update_thermal_coloring_rules(self):
        ...
        except Exception as e: print(f"Güncelleme sırasında hata: {e}")
    ...
    def update_thermal_data(self, data):
        try:
            ...
            if hotspot_node: self.get_calibrated_point(hotspot_node.get('positionX', 0), hotspot_node.get('positionY', 0), "hot")
            else: self.calibrated_hotspot_coords = None

            if coldspot_node: self.get_calibrated_point(coldspot_node.get('positionX', 0), coldspot_node.get('positionY', 0), "cold")
            else: self.calibrated_coldspot_coords = None
        except Exception as e:
            print(f"Termal JSON işleme hatası: {e}")
            
    def get_calibrated_point(self, thermal_x, thermal_y, point_type):
        xml_request = f"<PointRelation><srcPoint><positionX>{thermal_x}</positionX><positionY>{thermal_y}</positionY></srcPoint></PointRelation>"
        headers = {'Content-Type': 'application/xml'}
        try:
            ...
            if response.status_code == 200:
                ...
                if dest_point is not None:
                    cal_x = float(dest_point.find('isapi:positionX', ns).text)
                    cal_y = float(dest_point.find('isapi:positionY', ns).text)
                    if point_type == "hot": self.calibrated_hotspot_coords = (cal_x, cal_y)
                    elif point_type == "cold": self.calibrated_coldspot_coords = (cal_x, cal_y)
        except requests.exceptions.RequestException:
            # Hata durumunda basit oranlamayı kullan (daha az hassas ama program çalışır)
            if point_type == "hot": self.calibrated_hotspot_coords = (thermal_x, thermal_y)
            elif point_type == "cold": self.calibrated_coldspot_coords = (thermal_x, thermal_y)
