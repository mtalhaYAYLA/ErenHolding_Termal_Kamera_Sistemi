                ...
                while self._run_flag:
                    ...
                    if self.is_thermal:
                        ...
                        if self.parent_ui.thermal_roi_on_visible:
                            ...
                            # Koordinatlar 0-1000 aralığında geldiği için frame boyutuna göre oranla
...
class PTZControlApp(QWidget):
    ...
    def init_ui(self):
        ...
        thermal_box = QGroupBox("Bölge Sıcaklık Verileri")
        temp_info_layout = QFormLayout()
        self.temp_avg_label = QLabel("-")
        self.temp_min_label = QLabel("-")
        self.temp_max_label = QLabel("-")
        temp_info_layout.addRow("Ort. Sıcaklık:", self.temp_avg_label)
        temp_info_layout.addRow("Min. Sıcaklık:", self.temp_min_label)
        temp_info_layout.addRow("Maks. Sıcaklık:", self.temp_max_label)
    ...
    def calibrate_point(self, thermal_x, thermal_y):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if dest_point:
                    return (int(dest_point.find('isapi:positionX', ns).text), int(dest_point.find('isapi:positionY', ns).text))
        except:
