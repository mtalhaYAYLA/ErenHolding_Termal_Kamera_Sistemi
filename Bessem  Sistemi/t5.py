...
class PTZControlApp(QWidget):
    ...
    def init_ui(self):
        ...
        self.current_pos_label = QLabel("X: --°, Y: --°, Zoom: --%"); self.current_pos_label.setStyleSheet("font-weight: bold; color: #33aaff;")
        ...
        self.manual_speed_input = QLineEdit("0.9"); self.manual_speed_input.setValidator(QDoubleValidator(0.01, 1.0, 2))
        ...
        interval_grid = QFormLayout() 
        self.pan_start_input = QLineEdit("180"); interval_grid.addRow("Başlangıç Açısı (X1°):", self.pan_start_input)
        self.pan_end_input = QLineEdit("270"); interval_grid.addRow("Bitiş Açısı (X2°):", self.pan_end_input)
        self.tilt_fixed_input = QLineEdit("60"); interval_grid.addRow("Sabit Eğim (Y°):", self.tilt_fixed_input)
        self.scan_step_input = QLineEdit("10"); interval_grid.addRow("Adım Büyüklüğü (°):", self.scan_step_input)
        self.scan_wait_input = QLineEdit("10"); interval_grid.addRow("Bekleme Süresi (sn):", self.scan_wait_input)
        ...
        self.scan_step_input.setValidator(QDoubleValidator(1.0, 90.0, 1)); self.scan_wait_input.setValidator(QDoubleValidator(1.0, 600.0, 1))
        ...
        interval_scan_layout.addWidget(self.interval_scan_button)
    ...
    # YENİDEN EKLENDİ: Eksik olan fonksiyon
    ...
    def set_controls_enabled(self, enabled):
        ...
        ptz_widgets = [self.btn_up, self.btn_down, self.btn_left, self.btn_right, self.btn_zoom_in, self.btn_zoom_out, self.scan_360_button,
                       self.interval_scan_button, self.pan_start_input, self.pan_end_input, self.tilt_fixed_input, 
                       self.manual_speed_input, self.scan_step_input, self.scan_wait_input]
    ...
    def _convert_ptz_to_degrees(self, ptz_pos):
        pan_onvif = ptz_pos['X']; pan_deg = self._map_value(pan_onvif, 0, 1, 0, 180) if pan_onvif >= 0 else self._map_value(pan_onvif, -1, 0, 180, 360)
        ...
        return {'X': pan_deg, 'tilt': tilt_deg, 'zoom': zoom_percent}
    ...
    def update_ptz_status_display(self):
        ...
        try:
            ...
            if pos_raw and pos_raw.Position:
                pos_onvif = {'X': pos_raw.Position.PanTilt.x, 'tilt': pos_raw.Position.PanTilt.y, 'zoom': pos_raw.Position.Zoom.x}
                pos_deg = self._convert_ptz_to_degrees(pos_onvif)
                self.current_pos_label.setText(f"X : {pos_deg['X']:.1f}°, Y : {pos_deg['tilt']:.1f}°, Zoom: {pos_deg['zoom']:.0f}%")
    ...
    def toggle_interval_scan(self, checked):
        ...
        if checked:
            try:
                pan_start = float(self.pan_start_input.text().replace(',', '.')); pan_end = float(self.pan_end_input.text().replace(',', '.'))
                self.scan_params = {
                    'min_deg': min(pan_start, pan_end), 'max_deg': max(pan_start, pan_end),
                    'tilt_deg': float(self.tilt_fixed_input.text().replace(',', '.')),
                    'step_deg': float(self.scan_step_input.text().replace(',', '.')),
                    'wait_sec': int(self.scan_wait_input.text().replace(',', '.')),
                    'current_target_deg': min(pan_start, pan_end), 'direction': 1
                }
                self.scan_phase = "moving"; self.interval_scan_button.setText("Taramayı Durdur")
                self.execute_interval_scan_step()
            except ValueError as e:
                QMessageBox.warning(self, "Giriş Hatası", f"Lütfen geçerli sayısal değerler girin.\n{e}")
                self.interval_scan_button.setChecked(False); self.is_interval_scanning = False; self.scan_360_button.setEnabled(True)
        else:
    ...
    def execute_interval_scan_step(self):
        ...
        if self.scan_phase == "moving":
            print(f"Hedefe gidiliyor: {self.scan_params['current_target_deg']:.1f}°")
            self.go_to_degree(self.scan_params['current_target_deg'], self.scan_params['tilt_deg'])
            self.scan_phase = "waiting"; self.interval_scan_timer.start(self.scan_params['wait_sec'] * 1000)
        elif self.scan_phase == "waiting":
            current_target = self.scan_params['current_target_deg']; step = self.scan_params['step_deg']; direction = self.scan_params['direction']
            next_target = current_target + (step * direction)
            if next_target >= self.scan_params['max_deg']:
                self.scan_params['direction'] = -1; next_target = self.scan_params['max_deg']
            elif next_target <= self.scan_params['min_deg']:
                self.scan_params['direction'] = 1; next_target = self.scan_params['min_deg']
            self.scan_params['current_target_deg'] = next_target; self.scan_phase = "moving"
            self.interval_scan_timer.start(10)
    ...
    def go_to_target_position(self):
        # Bu fonksiyon artık doğrudan Adım Adım Tarama paneli için kullanılmıyor,
        # ancak gelecekteki bir "Tek Noktaya Git" özelliği için kalabilir.
        # Şimdilik devre dışı bırakmak yerine, geçici olarak bir mesaj gösterebiliriz.
        QMessageBox.information(self, "Bilgi", "Bu özellik şu anda 'Adım Adım Tarama' mantığına entegre edilmiştir.")
