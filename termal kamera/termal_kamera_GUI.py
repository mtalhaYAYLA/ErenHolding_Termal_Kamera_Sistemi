...
# Qt platformunu sadece Linux'ta xcb olarak ayarla
if platform.system() == 'Linux':
    os.environ['QT_QPA_PLATFORM'] = 'xcb'
elif platform.system() == 'Darwin':
    os.environ['QT_QPA_PLATFORM'] = 'cocoa'
# Windows için ayarlamaya gerek yok
...
# DEĞİŞTİRİLDİ: Dikdörtgenin zoom=0 iken başlangıç konumu ve boyutu.
# (x, y, genişlik, yükseklik) formatında. Bu değerleri değiştirerek başlangıç pozisyonunu ayarlayabilirsiniz.
EFFECTIVE_BASE_RECT = (240, 140, 160, 80) 

# DEĞİŞTİRİLDİ: Büyüme katsayısı sizin istediğiniz gibi yüksek. BU DEĞERİ AYARLAYABİLİRSİNİZ.
MAX_RECT_SCALE_FACTOR = 25.0 

# YENİ: Termal kameranın lensine ait Görüş Alanı (Field of View) dereceleri.
THERMAL_FOV_H = 50.0  # Yatay Görüş Alanı (Derece)
THERMAL_FOV_V = 150.0 # Dikey Görüş Alanı (Derece)
...
class RTSPVideoThread(QThread):
    ...
    def __init__(self, rtsp_url, is_thermal=False, parent=None):
        ...
        self.draw_rect_coords = EFFECTIVE_BASE_RECT if not is_thermal else None
    def run(self):
            ...
            try:
                ...
                while self._run_flag:
                    ...
                    if not self.is_thermal and self.draw_rect_coords:
                        ...
                        # DÜZELTİLDİ: Çizim komutu basitleştirildi. Tüm hesaplama ana sınıfta yapılıyor.
                        # Gelen koordinatlar olduğu gibi, ofset olmadan çizilmelidir.
                        cv2.rectangle(resized_frame, (x, y-20), (x + w, y + h), (0, 255, 0), 2)
                        
                        # DÜZELTİLDİ: Yazı, her zaman çizilen dikdörtgenin üzerinde olacak şekilde ayarlandı.
                        cv2.putText(resized_frame, 'Termal Goruntu Alani', (x, y - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                    ...
                    rgb_image = cv2.cvtColor(resized_frame, cv2.COLOR_BGR2RGB); h_img, w_img, ch = rgb_image.shape
                    qt_img = QImage(rgb_image.data, w_img, h_img, ch * w_img, QImage.Format_RGB888)
...
class PTZControlApp(QWidget):
    ...
    def init_ui(self):
        ...
        camera_layout.addWidget(log_box)
        ...
        right_panel_layout.addWidget(alarm_box); right_panel_layout.addStretch()
    ...
    def update_dynamic_rectangle(self, zoom_level):
        if not hasattr(self, 'thread_normal') or not self.thread_normal:
            return

        base_x, base_y, base_w, base_h = EFFECTIVE_BASE_RECT
        center_x = base_x + base_w / 2
        center_y = base_y + base_h / 2
        
        scale_factor = 1.0 + (zoom_level * (MAX_RECT_SCALE_FACTOR - 1.0))

        new_w = base_w * scale_factor
        new_h = base_h * scale_factor
    ...
    def calculate_hotspot_ptz(self, hotspot_coords):
        if self.last_ptz_status is None:
            return {'pan': 0, 'tilt': 0}

        try:
            current_pos_onvif = {
                'pan': self.last_ptz_status.PanTilt.x,
                'tilt': self.last_ptz_status.PanTilt.y,
                'zoom': self.last_ptz_status.Zoom.x
            }
            current_pos_deg = self._convert_ptz_to_degrees(current_pos_onvif)
            current_pan = current_pos_deg['pan']
            current_tilt = current_pos_deg['tilt']

            hotspot_x, hotspot_y = hotspot_coords

            pan_offset_deg = (hotspot_x - 0.5) * THERMAL_FOV_H
            tilt_offset_deg = (hotspot_y - 0.5) * THERMAL_FOV_V

            target_pan = (current_pan + pan_offset_deg) % 360
            target_tilt = current_tilt - tilt_offset_deg

            target_tilt = np.clip(target_tilt, self.TILT_DEGREE_RANGE[0], self.TILT_DEGREE_RANGE[1])

            return {'pan': target_pan, 'tilt': target_tilt}
        except Exception as e:
            print(f"Hotspot hesaplama hatası: {e}")
            return {'pan': 0, 'tilt': 0}
